...
namespace proc
{
    ...
    struct SleepingTask {
        struct Task *m_task;
        uint wakeup_tick;
        SleepingTask():m_task(nullptr), wakeup_tick(0) {}
        SleepingTask(struct Task *a_task, uint a_tick):m_task(a_task), wakeup_tick(a_tick) {}
        bool operator==(const struct SleepingTask &a_task) const { return m_task==a_task.m_task && wakeup_tick==a_task.wakeup_tick; }
    };

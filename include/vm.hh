...
namespace vm
{
    ...
    public:
        ...
        inline string toString() const{return klib::format("<VMO>@0x%lx[%lx]\n",ppn(),pages());}
    ...
    public:
        ...
        inline ByteArray copyinstr(xlen_t addr, size_t len) {
            ...
            return ByteArray((uint8_t*)buff, strlen(buff)+1);
        }
        inline ByteArray copyin(xlen_t addr,size_t len){
            // @todo 检查用户源是否越界（addr+len来自用户进程大小之外的空间）
            xlen_t paddr=pagetable.transaddr(addr);
            ByteArray buff((uint8_t*)paddr,len);
            return buff;
        }
        ...
        public:
            ...
            inline void operator=(const ByteArray &bytes){

...
static int sd_check_type() {
    ...
    printf("SD card replies cmd8 with status 0x%.2x", reply);
    if (reply & 0x04) {
        /* Illegal command */
        ...
        printf("SD card replies cmd8 with payload 0x%.8x", payload);
...
static void sd_reset() {
    ...
    printf("Set CS and MOSI to 1 and toggle clock.\n");
    ...
    printf("Set CS to 0 and send cmd0 to SD card.\n");
    char cmd0[] = {0x40, 0x00, 0x00, 0x00, 0x00, 0x95};
    char reply = sd_exec_cmd(cmd0);

    printf("Wait for SD card's reply to cmd0 (QEMU will stuck here)\n");
...
void sdinit() {
    ...
    sd_reset();
    // printf("Set SPI clock frequency to %ldHz", CPU_CLOCK_RATE / 4);
    // spi_set_clock(CPU_CLOCK_RATE / 4);

    printf("Check SD card type and voltage with cmd8\n");
    if (0 != sd_check_type()) panic("Fail to check SD card type\n");

    char acmd41[] = {0x69, (SD_CARD_TYPE == SD_TYPE_SD2)? 0x40 : 0x00, 0x00, 0x00, 0x00, 0xFF};
    while (sd_exec_acmd(acmd41));
    while (recv_data_byte() != 0xFF);

    printf("Set block size to 512 bytes with cmd16\n");
    char cmd16[] = {0x50, 0x00, 0x00, 0x02, 0x00, 0xFF};
    char reply = sd_exec_cmd(cmd16);
    while (recv_data_byte() != 0xFF);

    if (SD_CARD_TYPE == SD_TYPE_SD2) sd_check_capacity();
    if (SD_CARD_TYPE != SD_TYPE_SDHC) panic("Only SDHC/SDXC supported\n");
}

...
static void read_entry_name(char *buffer, union Ent *d) {
...
*/
// 将目录项d的信息读取到entry
static void read_entry_info(struct DirEnt *entry, union Ent *d) {
...
// 根据路径寻找目录（不进入该目录）
struct DirEnt *fs::entEnterParent(char *path, char *name) { return lookup_path(path, 1, name); }
// 根据路径寻找目录（进入该文件/目录）

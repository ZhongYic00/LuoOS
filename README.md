...
线程中封装有用户态上下文`Context ctx`和内核态上下文`kctx`，其中`kctx`额外存有当前线程的内核栈位置（内核空间地址）等。线程中还存有当前线程目前所处的特权级 `lastpriv` ，以供任务切换时进入正确的特权级。
...
- 有很小的概率初始化fat的时候在virtio上挂掉，通过logging和调试发现，挂掉的时候是由于第一次virtio读写之后没有产生外部中断（plic初始化正常，但sip里seip位一直是0）因此导致死循环
